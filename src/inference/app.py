"""
GUI ứng dụng nhận diện hành động bóng rổ (Basketball AI Pro) dùng
Tkinter, OpenCV và MediaPipe Pose + model SVM đã train.

Chức năng:
    - START / STOP camera
    - RECORD video kết quả (lưu vào docs/demo/)
    - Hiển thị action, confidence, FPS theo thời gian thực
...
# Video demo (record) sẽ lưu vào docs/demo/
RECORDS_DIR = os.path.join(BASE_DIR, "docs", "demo")
...
def toggle_record():
    ...
    if not recording:
        ...
        current_filename = os.path.join(RECORDS_DIR, filename)
...
def update_frame():
    ...
    cv2.putText(
        frame, f"Conf: {confidence:.2f}", (20, 90),
